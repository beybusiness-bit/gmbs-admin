...
- `loadBrandContracts` → `window.loadBrandContracts`로 전역 노출 (onclick 콜백 접근 오류 수정)
- `autoPromoteContractIfReady` 담당자 필드명 버그 수정: `pm.contact_email` → `pm.email`
- orphaned type UI 재설계: 오렌지 박스 제거 → 계약 목록 아이템으로 통합
- `saveNewContract`에 `no_contract: true` 플래그 저장 (계약서 없이 추가 시)
- `removeOrphanBrandType(brandId, contractType, contractId)`: contractId 있으면 계약 문서도 삭제
- `CONTRACT_STATUS_FILTERS`에 계약취소 추가

**아직 미해결 (다음 세션에서 처리):**
- `saveNewContract`에 `autoPromoteContractIfReady` 호출 누락 → 자동전환 여전히 안 됨
- no_contract 레거시 판별 조건 추가 (기존 PB 계약에 플래그 없음)
- 브랜드 상세 → 계약 상세 모달 닫기 시 브랜드 상세로 복귀 (현재 전체 모달 닫힘)
- `[계약서 없음]` 태그 표시
...
**DB 스키마 추가:**
```
settings/contract_field_rules
  {
    "위탁": ["business_type", "resident_number_or_biz_reg", "bank_account"],
    "PB":   ["business_type", "bank_account"],
    "매입": []
  }
```
