...
1~17단계 + 추가 UI 개선 완료. 18단계(전자계약 발송·상태 표시)는 gmbs-functions 콜러블 함수 완성 후 admin에 연결 예정.

**현재 브랜치:** `claude/gmbs-admin-dev-kes5rh` → Vercel 미리보기에서 확인 후 main 머지

**테스트 체크리스트 (미리보기 URL에서 확인):**
- 탭 타이틀이 "GMBS 관리자"로 표시되는지
- 사이드바 토글 버튼이 햄버거 아이콘(≡)으로 표시되는지
- 대시보드 "처리할 업무" 섹션이 0건일 때도 "처리할 업무 없음" 메시지와 함께 표시되는지
- 문의 목록에 "담당자" 컬럼이 표시되는지
- 문의 등록일이 "Timestamp(seconds=...)" 대신 정상 날짜/시간으로 표시되는지
- 이메일 설정 페이지에 "1:1 문의 답변 완료" 트리거가 표시되는지
- 전체 컬러가 보라색(#8c52ff) 계열로 변경됐는지

**테스트 완료 후:** main 머지 → 프로덕션 배포
