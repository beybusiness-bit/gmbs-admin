...
18단계 전자계약 발송·상태 표시(유캔사인) — 브랜드 상세 화면 "전자계약 발송" 버튼, contract_templates 관리, sendContractCallable 연결, 계약 상태 배지, signed_pdf_url 다운로드, 서류 URL 표시 — ✅ 완료
추가 보안 강화 — 민감정보(주민번호·계좌번호·사업자번호) 암호화 저장 및 on-demand 복호화, XSS 방어(esc 강화), 탭나빙 방어(rel=noopener), UCansign 콘솔로그 제거, 브랜드코드 중복 확인(승인 화면·신규 등록), 브랜드 타입 승인 화면 추가 — ✅ 완료
...
```
brands/{brandId}
  brand_name, brand_code, brand_type(PB|위탁|매입 등, 레거시—읽기 전용), brand_types(배열, 활성 계약 유형 자동 관리), active,
...
products/{productId}     // productId 예: "ONG-014"
  ...
  transaction_type(위탁|PB|매입|기타),  // 상품 등록 시 브랜드 brand_types 기반 선택
...
brands/{brandId}
  ...
  brands/{brandId}/contracts/{contractId}
    version, contract_type(위탁|PB|매입|기타), signed_at, end_at, auto_renew(bool),
    contract_status(계약정보입력|계약완료|계약기간만료|해지),  // admin 수동 처리용
    status(진행중|만료|해지, 레거시 호환용),  // ucansign webhook이 쓰는 필드 (functions만 write)
    file_url(Storage 경로), uploaded_at, uploaded_by
    // admin만 쓰기 가능. vendor는 읽기 전용 (다운로드만)
    // 계약 완료 시 brand_types에 contract_type 자동 추가, 만료/해지 시 자동 제거
...
#### 14-1. 브랜드 영구 삭제 차단 조건 (현재 구현)

브랜드 삭제(단건 `confirmBrandDelete` / 일괄 `bulkDeleteBrands`) 시 아래 조건 중 하나라도 해당되면 삭제를 차단하고 안내 모달을 표시한다.

| 차단 조건 | 판단 방법 | 메시지 |
|---|---|---|
| 브랜드에 소속된 상품이 1개 이상 있음 | `products` 컬렉션에서 `brand_id == brandId` 쿼리 | "상품 N개 있음" |
| 미결된 정산 내역이 있음 | `settlements` 컬렉션에서 `brand_id == brandId && pay_status in ['예정','검토중','확정']` 쿼리 | "미결 정산 N건 있음" |

**향후 추가 검토가 필요한 차단 조건** (정산·재고 기능이 더 완성된 후 세션에서 추가):
- 미정산 판매건(`sales` 컬렉션)이 있는 경우 — `sales` 컬렉션 구현 후 추가
- 출고되지 않은 재고가 남아 있는 경우 — `inventory_transactions` 구현 후 추가
- 진행 중인 전자계약(`ucansign_status in ['발송됨','서명진행중']`)이 있는 경우 — 현재는 경고 없이 삭제 허용, 추후 차단 여부 검토

**운영 원칙**: 삭제 대신 비활성화(`active: false`)를 기본으로 안내한다. 삭제는 테스트 데이터·오등록 건에 한해 사용하는 것을 권장.

#### 15. 향후 재고관리 확장 구조 (1차 미구현, 구조만 대비)
- **[변경: 2026-07-17 gmbs-functions 확인으로 기획 수정]** ~~"재고 원본 = Toss POS"~~ 원칙 폐기. Toss POS에 실시간 재고 조회 API가 없으므로 **`inventory_transactions` 컬렉션이 재고의 유일한 원본**이다.
- 거래 유형별 기록 주체:
  - **입고**: admin 담당자가 수동으로 `inventory_transactions`에 기록
  - **판매/환불 차감**: gmbs-functions가 Toss 웹훅을 받아서 자동 기록
  - **폐기/분실**: admin 수동 기록
- 재고 수량을 직접 덮어쓰지 않고 모든 변동을 누적 기록하는 방식으로 관리.
- 필드 예상: transaction_id, sku_id, unit_id(개체관리 상품만, 일반 상품은 빈칸), transaction_type(입고/판매/환불/폐기/분실), quantity(일반 상품용 수량 변화), transaction_date, reference_id, memo
...
1~18단계 + 추가 기능 전체 완료. 보안 강화(민감정보 암호화, XSS/탭나빙 방어 등)까지 완료.

**이번 세션에서 구현한 내용 (계약/브랜드 데이터 모델 정비):**
- `brand_type`(문자열) → `brand_types`(배열) 전환: 브랜드 폼, 목록, 뱃지 렌더 전체 반영
- `onboarding_status` 5개 값으로 단순화: 심사중/승인됨/거절/입점중/입점종료 (기존 복잡한 값 제거)
- `contract_status` 필드명 정규화 + `contract_type` 필드 추가(위탁/PB/매입/기타)
- 계약 완료 수동 처리 버튼(✅ 계약 완료 처리): 체결 시 `brand_types`에 해당 유형 자동 추가, `onboarding_status` 승인됨→입점중 자동 전환
- 계약 만료/해지 시 `brand_types`에서 해당 유형 자동 제거, 배열이 비면 입점중→입점종료 자동 전환
- 상품 등록 폼에 거래 유형(transaction_type) 선택 필드 추가: 브랜드의 brand_types 기반 동적 표시
- 입점계약관리 페이지 재편: CM_STATES를 onboarding_status 5개 값으로 교체, 체크박스 제거, 거래유형 컬럼 추가, 계약 정보 입력 버튼 추가
- 브랜드 상세 계약 섹션: "계약 정보 입력" 버튼 추가, contract_type 뱃지 표시, contract_status 뱃지 표시

**현재 브랜치:** `claude/contract-template-variables-h4va9w`

**다음 세션 할 일:**
- 브랜치 push 후 PR 머지
- 이번 세션 구현 테스트 후 피드백 반영
- (선택) brand_types 자동 업데이트가 UCansign 체결완료(functions webhook) 경로에서도 동작하는지 확인 → functions 쪽에서 계약 status 변경 시 admin과 동일한 addContractTypeToBrand 로직 추가 필요
