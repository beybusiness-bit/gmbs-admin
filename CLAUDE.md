...
const REPO = {
  ...
  LIVE_URL: 'https://beybusiness-bit.github.io/gmbs-admin/', // GitHub Pages 활성화 후 실제 URL로 확정
};
```
...
3. 승인 후:
   ...
   - push 완료 후: "✅ 푸시 완료. GitHub Pages 반영까지 ~1분. Cmd+Shift+R 하드 리프레시 해주세요."
...
배포 방식: github-pages (main 직접 push, PR 불필요)
...
3. Firebase 콘솔 → Authentication → 승인된 도메인에 GitHub Pages URL(`beybusiness-bit.github.io`) 추가
...
1단계 프로젝트 셋업(Firebase 연결, Google 로그인, 사이드바 레이아웃, PWA manifest) — 🔲 진행 예정
2단계 브랜드 관리(등록/수정/사용중지, 브랜드코드 중복확인) — 🔲 진행 예정
3단계 상품 등록 ①(기본정보+옵션 설정, 상품번호 자동 미리보기, 옵션 최대 3축, 조합 자동생성, 순서 드래그) — 🔲 진행 예정
4단계 상품 등록 ②(트랜잭션 기반 번호 발급·중복검증 저장, 개체관리 상품 개체 생성, 최종 확인 화면) — 🔲 진행 예정
5단계 상품 목록·검색·수정(상품 단위 묶음 표시, 브랜드/상품명/바코드 검색, 판매중지 처리) — 🔲 진행 예정
6단계 개체 관리(SKU별 개체 목록, 상태 변경: 판매중/판매완료/분실/폐기) — 🔲 진행 예정
7단계 바코드 생성·출력(Code128, 라벨 미리보기, 동일SKU 반복/개체 연속 출력, PNG/ZIP 다운로드, 브라우저 인쇄·PDF) — 🔲 진행 예정
8단계 설정 페이지(번호 자릿수 규칙 및 변경 경고, 라벨 크기, 데이터 관리 엑셀 내보내기, Firebase 연결상태) — 🔲 진행 예정
9단계 PWA 마무리(아이콘, service worker, 홈 화면 설치 안내) — 🔲 진행 예정
...
- **[변경: 서버 불필요 판단]** 원본 제안서는 "Next.js 서버 + Google Sheets API + Vercel 배포"를 명시했으나, Firestore 트랜잭션이 번호 중복 방지를, Firestore 보안 규칙이 인증정보 보호를 대체할 수 있다고 판단해 **단일 HTML + Firebase(Firestore/Auth) + GitHub Pages**로 변경. 데이터 모델·번호 체계·바코드 규칙·화면 흐름은 원본 그대로 유지.
...
1단계 — 프로젝트 셋업(Firebase 연결, Google 로그인, 사이드바 레이아웃, PWA manifest)부터 시작
