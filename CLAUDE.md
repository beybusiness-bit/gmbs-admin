...
1~17단계 전체 구현 완료 (로컬 커밋 완료, push 대기 중).

**push 전 테스트 필요 항목 (push 후 Vercel 미리보기 URL에서 확인):**
- 10단계: 브랜드 상세 팝업 > 담당자 섹션 — 추가/수정/비활성화
- 11단계: 브랜드 상세 팝업 > 계약 섹션 — PDF 업로드/다운로드/상태 변경
- 12단계: 브랜드 목록 입점상태 필터 + 브랜드 상세 팝업 > 상태 변경
- 13단계: 상품 목록 승인상태 필터 + 상품 상세 팝업 > 승인/거절/정가 자동산정
- 14단계: 사이드바 > 정산 메뉴 — 목록/상세/지급상태/증빙상태 변경
- 15단계: 브랜드 상세 팝업 > 변경 이력 섹션 (상태 변경 동작 후 이력 기록 확인)
- 16단계: 사이드바 > 공지사항 — 작성/수정/게시
- 17단계: 사이드바 > 문의 — 목록/상세/답변 작성

**테스트 완료 후:** `git push -u origin claude/gmbs-admin-dev-kes5rh` → Vercel 미리보기 URL 확인 → main 머지 → 프로덕션 배포
